...
public class SongOfAquaticGrace extends Skill {
	
	public static String skillName = "Song Of Aquatic Grace";
	public static Material skillMaterial = Material.PUFFERFISH_SPAWN_EGG;
	...
	public static ItemStack getItem(Player player) {
		...
		loreList.add("§aEffect §7: Increase the target's swimming");
	...
	public SongOfAquaticGrace(LivingEntity source, ArrayList<LivingEntity> targets) {
	...
	public void onEffect() {
		...
		for (LivingEntity e : nearby) {
			...
			PotionEffect effect = e.getPotionEffect(PotionEffectType.DOLPHINS_GRACE);
			...
			if (!(e.getName().equals(source.getName())))
				Particles.Trail(e.getEyeLocation().clone().add(0, -0.25F, 0), source.getLocation().clone().add(0, -0.25F, 0), nb_particle, Particle.NOTE, 14 / 24D);

			int duration = skillEffectDuration;
			if (source instanceof Player)
				duration *= RoCPlayers.getEffectDurationAugmentation(((Player) source));
			UtilFuncs.buffEntity(e, PotionEffectType.DOLPHINS_GRACE, 1, duration);

		    BukkitRunnable note1 = new  BukkitRunnable() {
			    @Override
			    public void run() {
					e.getWorld().playSound(e.getLocation(), Sound.BLOCK_NOTE_BLOCK_HARP, 0.2F, 0.4F);
			    }
			};
		    BukkitRunnable note2 = new  BukkitRunnable() {
			    @Override
			    public void run() {
			    	e.getWorld().playSound(e.getLocation(), Sound.BLOCK_NOTE_BLOCK_HARP, 0.2F, 0.6F);
			    }
			};
		    ...
		    BukkitRunnable note4 = new  BukkitRunnable() {
			    @Override
			    public void run() {
			    	e.getWorld().playSound(e.getLocation(), Sound.BLOCK_NOTE_BLOCK_HARP, 0.2F, 0.8F);
			    }
			};
