...
public class SongOfFeathers extends Skill {
	
	public static String skillName = "Song Of Feathers";
	public static Material skillMaterial = Material.SKELETON_HORSE_SPAWN_EGG;
	...
	public static ItemStack getItem(Player player) {
		...
		loreList.add("§aEffect §7: Decrease the target's falling");
	...
	public SongOfFeathers(LivingEntity source, ArrayList<LivingEntity> targets) {
	...
	public void onEffect() {
		...
		for (LivingEntity e : nearby) {
			...
			PotionEffect effect = e.getPotionEffect(PotionEffectType.SLOW_FALLING);
    		if (effect != null && (effect.getAmplifier() == 0 || effect.getAmplifier() == 2)) continue;
    		// ^ If effect exists and amplifier is stage 1 or 3
			
			int nb_particle = (int) source.getLocation().distance(e.getEyeLocation());
			if (!(e.getName().equals(source.getName())))
				Particles.Trail(e.getEyeLocation().clone().add(0, -0.25F, 0), source.getLocation().clone().add(0, -0.25F, 0), nb_particle, Particle.NOTE, 10 / 24D);

			int duration = skillEffectDuration;
			if (source instanceof Player)
				duration *= RoCPlayers.getEffectDurationAugmentation(((Player) source));
			UtilFuncs.buffEntity(e, PotionEffectType.SLOW_FALLING, 1, duration);

		    BukkitRunnable note1 = new  BukkitRunnable() {
			    @Override
			    public void run() {
					e.getWorld().playSound(e.getLocation(), Sound.BLOCK_NOTE_BLOCK_FLUTE, 0.2F, 0.5F);
			    }
			};
		    BukkitRunnable note2 = new  BukkitRunnable() {
			    @Override
			    public void run() {
			    	e.getWorld().playSound(e.getLocation(), Sound.BLOCK_NOTE_BLOCK_FLUTE, 0.2F, 0.7F);
			    }
			};
		    ...
		    BukkitRunnable note4 = new  BukkitRunnable() {
			    @Override
			    public void run() {
			    	e.getWorld().playSound(e.getLocation(), Sound.BLOCK_NOTE_BLOCK_FLUTE, 0.2F, 0.9F);
			    }
			};
		    BukkitRunnable note5 = new  BukkitRunnable() {
			    @Override
			    public void run() {
					e.getWorld().playSound(e.getLocation(), Sound.BLOCK_NOTE_BLOCK_FLUTE, 0.2F, 0.3F);
			    }
			};
			note1.runTaskLater(RealmsOfChaos.getInstance(), 1);
			note2.runTaskLater(RealmsOfChaos.getInstance(), 4);
			note3.runTaskLater(RealmsOfChaos.getInstance(), 7);
			note4.runTaskLater(RealmsOfChaos.getInstance(), 10);
			note5.runTaskLater(RealmsOfChaos.getInstance(), 13);
		}
	}
