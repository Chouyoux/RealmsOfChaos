...
public class Cowardise extends Skill {
	
	public static String skillName = "Cowardise";
	...
	public static ItemStack getItem(Player player) {
		...
		loreList.add("§aEffect §7: Reduces the movement speed");
		loreList.add("§7of the target by 3 stages if the user");
		loreList.add("§7sees its back.");
	...
	public Cowardise(LivingEntity source, ArrayList<LivingEntity> targets) {
	...
	public void onEffect() {
		Vector target_direction = targets.get(0).getLocation().getDirection();
		Vector source_direction = source.getLocation().getDirection();
		if (Math.toDegrees(target_direction.angle(source_direction)) < 90) {
			source.getWorld().playSound(source.getLocation(), Sound.ENTITY_CHICKEN_HURT, 0.5F, 0.5F);
			targets.get(0).getWorld().playSound(targets.get(0).getLocation(), Sound.ENTITY_CHICKEN_HURT, 0.5F, 0.5F);
			targets.get(0).addPotionEffect(new PotionEffect(PotionEffectType.SLOW, skillEffectDuration, 2));
		}
	}
