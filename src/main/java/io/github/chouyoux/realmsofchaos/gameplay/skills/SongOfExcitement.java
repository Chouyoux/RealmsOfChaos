...
public class SongOfExcitement extends Skill {
	
	public static String skillName = "Song Of Excitement";
	public static Material skillMaterial = Material.PIG_SPAWN_EGG;
	...
	public static ItemStack getItem(Player player) {
		...
		loreList.add("§aEffect §7: Increase the target's jump");
		loreList.add("§7force by one stage.");
	...
	public SongOfExcitement(LivingEntity source, ArrayList<LivingEntity> targets) {
	...
	public void onEffect() {
		...
		for (LivingEntity e : nearby) {
			...
			PotionEffect effect = e.getPotionEffect(PotionEffectType.JUMP);
			...
			if (!(e.getName().equals(source.getName())))
				Particles.Trail(e.getEyeLocation().clone().add(0, -0.25F, 0), source.getLocation().clone().add(0, -0.25F, 0), nb_particle, Particle.NOTE, 6 / 24D);

			int duration = skillEffectDuration;
			if (source instanceof Player)
				duration *= RoCPlayers.getEffectDurationAugmentation(((Player) source));
			UtilFuncs.buffEntity(e, PotionEffectType.JUMP, 1, duration);

		    BukkitRunnable note1 = new  BukkitRunnable() {
			    @Override
			    public void run() {
					e.getWorld().playSound(e.getLocation(), Sound.BLOCK_NOTE_BLOCK_GUITAR, 0.2F, 0.8F);
			    }
			};
		    BukkitRunnable note2 = new  BukkitRunnable() {
			    @Override
			    public void run() {
			    	e.getWorld().playSound(e.getLocation(), Sound.BLOCK_NOTE_BLOCK_GUITAR, 0.2F, 1F);
			    }
			};
		    BukkitRunnable note3 = new  BukkitRunnable() {
			    @Override
			    public void run() {
					e.getWorld().playSound(e.getLocation(), Sound.BLOCK_NOTE_BLOCK_GUITAR, 0.2F, 1.2F);
			    }
			};
